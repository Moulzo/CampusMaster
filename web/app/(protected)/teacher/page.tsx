...
export default function TeacherPage() {
  ...
  const subjectsLabel = useMemo(
    () => (statsLoading ? "…" : String(subjectsCount)),
    [statsLoading, subjectsCount]
  );
  const studentsLabel = useMemo(
    () => (statsLoading ? "…" : String(studentsCount)),
    [statsLoading, studentsCount]
  );
  const assignmentsLabel = useMemo(
    () => (statsLoading ? "…" : String(assignmentsCount)),
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <details className="bg-slate-900 rounded-lg p-4 text-slate-400 text-xs font-mono">
                <summary className="cursor-pointer font-bold text-slate-300 mb-3">
                  Données Utilisateur
                </summary>
